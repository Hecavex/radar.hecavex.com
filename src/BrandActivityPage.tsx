  ...
  return (
    <div className="site-shell">
      ...
      <main className="permanent-page brand-activity-page" id="main-content">
        ...
        {(data.historyTotal ?? data.history.length) > data.history.length && <p className="boundary-note">{lt ? `Čia pateikiami ${data.history.length} naujausių įrašų iš ${data.historyTotal}. Visą išsaugotą istoriją rasite archyve.` : `Showing ${data.history.length} recent records of ${data.historyTotal}. The complete retained record remains in the history archive.`} <a href="/history/">{lt ? "Visa istorija" : "Complete history"} →</a></p>}
